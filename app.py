...
def reset_database():
    """Drops and recreates all tables using schema.sql."""
    conn = config.get_database_connection()
    cursor = conn.cursor()
    try:
        cursor.execute("DROP TABLE IF EXISTS chat_history")
        cursor.execute("DROP TABLE IF EXISTS user_profiles")
        cursor.execute("DROP TABLE IF EXISTS segmented_nodes")
        conn.commit()
        config.initialize_sqlite_schema(conn)
        print(f"{Fore.GREEN}資料庫已成功重置！所有的對話紀錄、使用者偏好、分段節點皆已清除並重新初始化。")
    except Exception as e:
        print(f"{Fore.RED}重置資料庫失敗: {e}")
    finally:
        conn.close()
...
def run_local_demo():
    ...
    print(f"\n{Fore.YELLOW}[Step 2] 初始化本地數據庫與用戶偏好 Profile...")
    conn = config.get_database_connection()
